...
type Kanji struct {
	...
	Nanori      []string   `xml:"reading_meaning>nanori" json:"nanori,omitempty"`
}
...
func main() {
	if len(os.Args) != 2 {
		log.Fatal("Path to KANJIDIC2 XML file is required as argument.")
	}
	kanjidic2 := ParseKanjiDic2(os.Args[1])
	b, err := json.Marshal(kanjidic2)
	if err != nil {
		fmt.Println(err)
	}
